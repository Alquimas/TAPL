...
open Format
open String
open Fullomega.Support.Pervasive
open Fullomega.Support.Error
open Fullomega.Syntax
open Fullomega.Core

module Lexer = Fullomega.Lexer
module Parser = Fullomega.Parser
...
let rec process_file f (ctx,store) =
  if (f = "repl") then
    ...
    let cmds,_ = parseString text ctx in
      process_file "repl" (List.fold_left process_cmds (ctx, store) cmds))
      with End_of_file -> print_endline ""; (ctx, store)
      | _ -> process_file "repl" (ctx, store);
    ...
    in
      List.fold_left process_cmds (ctx,store) cmds)

and process_cmds (ctx,store) c =  
  open_hvbox 0;
